...
class GameState:
    ...
    def apply_move(self, move: Move) -> "GameState":
        ...
        # Also, check if rook involved in castling move (implicit) - already handled by move.start check if rook moves.
        # But if we castle, rights are revoked by King move.
        ...
        # Clean invalid rights (e.g. if rook captured/moved earlier but logic missed it, or just consistency)
        # Actually logic above covers moves/captures. 
        # But we also need to ensure we don't have rights if pieces are missing (e.g. from FEN manipulation).
        # We can perform a validation pass or trust the logic.
        # Let's trust logic for transition, but maybe validate on FEN load.
        
        return GameState(
            board=new_board,
            turn=self.turn.opposite,
            castling_rights=tuple(sorted(new_castling_rights, key=lambda x: x.value)), # Sort for stability
    ...
    def status(self) -> StatusReason:
        ...
        # Check if previous player left king in check
        # Turn is already switched in this state?
        # Usually 'is_legal' checks if the side NOT to move is in check (captured king).
        # Wait, if I make a move, I produce a state where it is OPPONENT'S turn.
        # If I left MY king in check, then `inactive_player_in_check` (my king) is true.
