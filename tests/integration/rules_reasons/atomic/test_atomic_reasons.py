...
def test_atomic_board_legality_kings_adjacent():
    ...
    assert rules.validate_board_state(state) == BoardLegalityReason.KINGS_ADJACENT
...
def test_atomic_move_legality_king_exploded_own():
    ...
    assert rules.validate_move(state, move) == MoveLegalityReason.KING_EXPLODED

def test_atomic_game_over_king_exploded():
    rules = AtomicRules()
    fen = "rnbqkbnr/pppppppp/8/8/8/8/8/RNBQ1BNR w KQkq - 0 1" # No white king
    state = state_from_fen(fen)
    assert rules.get_game_over_reason(state) == GameOverReason.KING_EXPLODED
