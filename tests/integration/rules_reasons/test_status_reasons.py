...
def test_status_reason_valid():
    ...
    rules = StandardRules()
    assert rules.validate_board_state(state) == BoardLegalityReason.VALID
...
def test_status_reason_opposite_check():
    ...
    state = GameState.from_fen(fen)
    rules = StandardRules()
    assert rules.validate_board_state(state) == BoardLegalityReason.OPPOSITE_CHECK